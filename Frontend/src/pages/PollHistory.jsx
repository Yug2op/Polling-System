      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-12">
          ...
          <button
            ...
            className="absolute top-8 right-8 text-white px-6 py-2 rounded-full text-sm font-bold shadow-lg"
          >
            ← Back
          </button>
        </div>
        ...
        {polls.length === 0 ? (
          ...
          <div className="space-y-16">
            {polls.map((poll, index) => {
              ...
              return (
                <div key={poll._id}>
                  <h2 className="text-xl font-bold text-gray-800 mb-6">Question {index + 1}</h2>
                  
                  <div className="border-2 border-gray-300 rounded-lg max-w-2xl overflow-hidden shadow-sm bg-white">
                    <div className="bg-gradient-to-r from-[#5C5C5C] to-[#6E6E6E]  p-4 flex justify-between items-center">
                      <p className="text-white font-medium text-sm">{poll.question}</p>
                      <span className="text-[10px] text-gray-300 font-mono">ID: {poll._id}</span>
                    </div>
                    ...
                    <div className="p-6 space-y-4">
                      {poll.options.map((opt, i) => {
                        ...
                        return (
                          <div key={i} className="relative h-12 border border-gray-100 rounded-xl overflow-hidden flex items-center bg-gray-50/30">
                            {/* Fill Bar */}
                            <div 
                            ...
                            <div className="absolute inset-0 flex items-center justify-between px-4 z-10">
                              <div className="flex items-center gap-3">
                                ...
                                {opt.isCorrect && (
                                  <div className={`ml-2 flex items-center justify-center w-4 h-4 rounded-full bg-green-500 text-white`}>
                              ...
                              <div className="flex items-center gap-3">
                                <span className={`text-xs ${percentage > 20 ? 'text-white/80' : 'text-gray-400'}`}>
                                  ({opt.votes} votes)
                                </span>
                                <span className={`text-sm font-bold ${percentage > 20 ? 'text-white' : 'text-gray-800'}`}>
                                  {percentage}%
                                </span>
                              </div>
                            </div>
